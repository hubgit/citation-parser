...
package ParaTools::CiteParser::Jiao; 

#use strict;
use vars qw(@ISA @EXPORT @EXPORT_OK);
# This provides various utility functions.
use ParaTools::CiteParser::Jiao::Utility; 
...
B<ParaTools::CiteParser::Jiao> - citation parsing using Zhuoan Jiao's
parsing module. 
...
$parser = new ParaTools::CiteParser::Jiao;
...
ParaTools::CiteParser::Jiao uses a reference parsing module written by
...
=item $parser = ParaTools::CiteParser::Jiao-E<gt>new()
...
sub parse
{
	my($self, $ref, $trim) = @_;
	my $cite = {};
	bless $cite, "ParaTools::CiteParser::Jiao";
	$cite->initialize($ref);
	$cite->{auinit} = $cite->{aufirst};
	$cite->{date} = $cite->{year};
	$cite = _debless($cite);
	$trim = 1;
	my $metadata = ($trim ? $cite : ParaTools::OpenURL::trim_openurl($cite));
...
sub find_vol_no_pg_year {
    ...
    return 0
        ...
        sub find_jnl_name {
            ...
            #
            my $Text = substr($cite->{'rest_text'}, 0, $cite->{'jnl_epos'} + 1); # alf added +1 30/04/2006
            my $Guess_jnl;
...
Ported to ParaTools interface by:
